...
final case class TreeZipper[A](tree: Tree[A], lefts: TreeForest[A],
  ...
  def map[B](f: A => B): TreeZipper[B] = {
    ...
    TreeZipper(tree map f, lefts map ff, rights map ff,
      parents.map {
  ...
  def cojoin: TreeZipper[TreeZipper[A]] = {
    ...
    def dwn[A](tz: TreeZipper[A]): (TreeZipper[A], () => Stream[TreeZipper[A]]) = {
      val f = () => unfoldStream(tz.firstChild) {
        o => for (c <- o) yield (c, c.right)
      }
      (tz, f)
    }
    def uf[A](a: TreeZipper[A], f: TreeZipper[A] => Option[TreeZipper[A]]): Stream[Tree[TreeZipper[A]]] = {
      unfoldStream(f(a)) {
        o => for (c <- o) yield (Tree.unfoldTree(c)(dwn[A](_: TreeZipper[A])), f(c))
      }
    }

    val p = unfoldStream(parent) {
      o => for (z <- o) yield ((uf(z, lft), z, uf(z, rgt)), z.parent)
    }
