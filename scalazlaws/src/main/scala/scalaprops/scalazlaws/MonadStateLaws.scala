...
private[scalazlaws] object MonadStateLaws {
  def apply[F[_, _], S](implicit F: MonadState[F, S]): MonadStateLaws[F, S] = new MonadStateLaws(F)
}

private[scalazlaws] class MonadStateLaws[F[_, _], S] private (F: MonadState[F, S]) {
